...
def setup_environment(seed=222):
...
def emission_probability(state, observation,sigma):
    ...
    x, y = position
    prob = -0.5 * (((obs_x - x) ** 2 + (obs_y - y) ** 2) / sigma ** 2)
    return prob
    # prob = -0.5 * (((obs_x - x) ** 2 + (obs_y - y) ** 2) / sigma ** 2) - np.log(2 * np.pi * sigma ** 2)
    # return prob
...
def transition_probability(prev_state, curr_state, movement_policy):
    ...
    if movement_policy == 'random_walk':
        return np.log(1 / len(HEADINGS)) if prev_pos != curr_pos else -np.inf
...
def viterbi(observations, start_state, movement_policy,states,sigma):
    ...
    dp[0][start_state] = 0.0  # Log probability of 1
    # for state in states:
    #     dp[0][state] = 0.0 if state == start_state else -np.inf

    backpointer[0][start_state] = None

    for t in range(1, T):
        for curr_state in states:
            max_prob, best_prev_state = -np.inf, None
            for prev_state in states:
                if prev_state in dp[t - 1]:
                    prob = (dp[t - 1][prev_state]
                            + transition_probability(prev_state, curr_state, movement_policy)
                            + emission_probability(curr_state, observations[t], sigma))
                    if prob > max_prob:
    ...
    # Backtracking
    max_final_prob, best_final_state = max((dp[T - 1][state], state) for state in dp[T - 1])
...
def save_paths_to_csv(seed_values, paths, filename="estimated_paths.csv"):
    with open(filename, 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(['Seed Value',"Policy", 'Estimated Path'])
...
if __name__ == "__main__":
    ...
    seed_values = [222, 123, 333]  # Test with different seeds
    estimated_paths = []          # Store paths for each seed

    for seed in seed_values:
        seed_everything(seed)
        for policy in policies:
            true_positions, headings, observations = simulate_roomba(T, policy, sigma)
            estimated_path = viterbi(
                observations,
                (true_positions[0], headings[0]),  # Start state
                policy,
                states,
                sigma
            )
    ...
    # Save paths to CSV
    save_paths_to_csv(seed_values, estimated_paths)
